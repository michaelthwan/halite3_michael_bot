...
def search_best_dropoff_point(game_map, sorted_cells, game_turn_number, enemy_ship_positions):
    NEIGHBORHOOD_DIST = 6
    ...
    for cell in sorted_cells:
        ...
        for neighbour_pos in get_area_by_archor_and_dist(game_map, NEIGHBORHOOD_DIST, {(pos.x, pos.y): (None, None)}):
            if neighbour_pos in enemy_ship_positions:
                enemy_count += 1
            neighbour_value += game_map[neighbour_pos].halite_amount

        neighbour_value = neighbour_value * (1 - 0.02 * enemy_count)
...
def append_f_log(f_log: list, t, x, y, msg) -> list:
    f_log.append({'t': t, 'x': x, 'y': y, 'msg': msg})
    return f_log
...
def should_build_ship(me, game, game_map, avg_halite_all_cell, real_time_halite):
    MIN_PROFITABLE_AVG_HALITE = 50
    is_under_ship_amount_cap = len(me.get_ships()) < MAX_NUM_SHIP
    have_halite = real_time_halite >= constants.SHIP_COST
    ...
    is_profitable = avg_halite_all_cell > MIN_PROFITABLE_AVG_HALITE
    return is_under_ship_amount_cap and have_halite and have_halite_extra_for_dropoff and is_friendly_not_in_shipyard and is_under_turn_cap and is_profitable


def should_build_dropoff(game, game_map, me):
    MIN_NUM_SHIP_PER_DROPOFF = 15
    is_under_max_dropoff_num = len(dropoff_dict) - 1 < MAX_NUM_DROPOFF
    is_ship_num_appropriate = len(me.get_ships()) / MIN_NUM_SHIP_PER_DROPOFF > len(dropoff_dict) - 1
    is_appropriate_turn = MIN_DROPOFF_TURN <= game.turn_number <= constants.MAX_TURNS - game_map.height * 0.75 - DROPOFF_TURN_FINAL_RETURN_GAP
    is_eval_turn = True
    # is_eval_turn = game.turn_number % 5 == 1
    has_enough_halite = me.halite_amount > constants.DROPOFF_COST
    is_not_cooldown = game.turn_number - last_dropoff_turn >= 20
    return is_under_max_dropoff_num and is_appropriate_turn and is_eval_turn and has_enough_halite and is_not_cooldown


def get_final_return_turn(ship, closest_dropoff_pos, game_map):
    return constants.MAX_TURNS - game_map.calculate_distance(ship.position, closest_dropoff_pos)


def find_enemy_positions(game):
    enemy_ship_positions = []
    enemy_dropoff_positions = []
    for player_id in range(len(game.players)):
        if game.my_id != player_id:
            ...
            enemy_dropoffs = game.players[player_id].get_dropoffs()
            for enemy_dropoff in enemy_dropoffs:
                enemy_dropoff_positions.append(enemy_dropoff.position)
    return enemy_ship_positions, enemy_dropoff_positions
...
try:
    while True:
        ...
        real_time_halite = me.halite_amount

        command_queue = []
        ship_next_step_list = {}
        enemy_potential_step_list = {}
        enemy_ship_positions, enemy_dropoff_positions = find_enemy_positions(game)
        ...
        avg_halite_all_cell = sum(
            [game_map[Position(x, y)].halite_amount for x in range(game_map.width) for y in range(game_map.height)]
        ) / (game_map.width * game_map.height)
        logging.info("total_ship_num : {}, avg_halite: {}, avg_halite(ship#skip10): {}, dropoff_list: {}".format(
            total_ship_num, round(avg_halite_all_cell, 2), round(avg_halite_ship_num_skip10, 2), dropoff_dict))
        logging.info("sorted_cells: {}".format([(c[0].x, c[0].y, round(c[1], 2)) for c in sorted_explorable_cells[:100]]))

        # Per turn update
        # for ship in me.get_ships():
        #     if ship.id in ship_status and ship_status[ship.id] == STATE_EXPLORE:
        #         del ship_target[ship.id]
        # for ship in me.get_ships():
        #     if ship.id in ship_status and ship_status[ship.id] == STATE_EXPLORE and ship.id not in ship_target:
        ...
        # Target switch
        all_ship_ids = [ship.id for ship in me.get_ships()]
        logging.info("all_ship_ids:{}".format(all_ship_ids))
        for key, value in ship_status.items():
            if value != STATE_EXPLORE or key not in all_ship_ids:
                continue
            ship1_id = key
            ship1_pos = me.get_ship(ship1_id).position
            ship1_target = ship_target[ship1_id]
            for key2, value2 in ship_status.items():
                if value2 != STATE_EXPLORE or key2 not in all_ship_ids:
                    continue
                ship2_id = key2
                ship2_pos = me.get_ship(ship2_id).position
                ship2_target = ship_target[ship2_id]
                if ship1_id == ship2_id:
                    continue
                ship1to_t1 = game_map.calculate_distance(ship1_pos, ship1_target)
                ship2to_t2 = game_map.calculate_distance(ship2_pos, ship2_target)
                ship1to_t2 = game_map.calculate_distance(ship1_pos, ship2_target)
                ship2to_t1 = game_map.calculate_distance(ship2_pos, ship1_target)

                if ship1to_t1 + ship2to_t2 > ship1to_t2 + ship2to_t1:
                    logging.info("Ship1:({}, {}, {}) ship1to_t1:{}, Ship2:({}, {}, {}) ship2to_t2:{}. Better swap: ship1to_t2:{}, ship2to_t1:{}".format(
                        ship1_id, ship1_pos, ship1_target, ship1to_t1, ship2_id, ship2_pos, ship2_target, ship2to_t2, ship1to_t2, ship2to_t1
                    ))
                    ship_target[ship1_id] = ship2_pos
                    ship_target[ship2_id] = ship1_pos
        ...
        # Create dropoff
        EVAL_NUM_DROPOFF = 10
        if should_build_dropoff(game, game_map, me):
            best_dropff_points = search_best_dropoff_point(game_map, sorted_explorable_cells[:EVAL_NUM_DROPOFF], game.turn_number, enemy_ship_positions)
        ...
        for ship in [ship for ship in me.get_ships() if ship not in commanded_ships]:
            ...
            final_return_turn = get_final_return_turn(ship, closest_dropoff_pos, game_map)
            # No energy
            if is_not_enough_fuel(ship, game_map):
            ...
            # Final return
            elif game.turn_number >= final_return_turn - 5:
                commanded_ships.append(ship)
                log_action("FinalReturn({})->{}".format(final_return_turn, closest_dropoff_pos), ship, game_map)
        ...
        for ship in [ship for ship in me.get_ships() if ship not in commanded_ships]:
            if ship_status[ship.id] == STATE_DROPOFF:
                if ship_target[ship.id] not in enemy_dropoff_positions:
                    if ship.position == ship_target[ship.id]:
                        if me.halite_amount > constants.DROPOFF_COST:
                            ...
                            saved_halite_for_dropoff -= constants.DROPOFF_COST
                            real_time_halite -= constants.DROPOFF_COST
                    ...
                    ship_status[ship.id] = STATE_EXPLORE
                    ship_target[ship.id] = search_best_expected_return_target(game_map, game.turn_number, ship, avg_halite_ship_num_skip10, sorted_explorable_cells)
                    log_action("DropoffBlockStartExplore->{}".format(ship_target[ship.id]), ship, game_map)
                    command_queue = move_ship_to_position(command_queue, game_map, ship, ship_target[ship.id], False, None)
                    continue
        # Mine
        for ship in [ship for ship in me.get_ships() if ship not in commanded_ships]:
        ...
        if should_build_ship(me, game, game_map, avg_halite_all_cell, real_time_halite) or is_enemy_blocking_shipyard(me, enemy_ship_positions):
